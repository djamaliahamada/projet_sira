<?php
    ...
    if(isset($_GET['id'])){
    $id=$_GET['id'];
    $query = "SELECT vehicule.id_vehicule, vehicule.marque, vehicule.modele, vehicule.prix_journalier, 
              vehicule.description, vehicule.image
              FROM vehicule 
              WHERE vehicule.id_vehicule = $id";
    $vehicules = $pdo->prepare($query);
    $vehicules->execute();
    $location = $vehicules->fetch();
}
